...
else{var t=function(w,B,E,F){F.action.call(B.button(E),w,B,E,F);f(B.table().node()).triggerHandler("buttons-action.dt",[B.button(E),B,E,F])};h=a.tag||k.tag;var A=a.clickBlurs===q?!0:a.clickBlurs;g=f("<"+h+"/>").addClass(k.className).addClass(d?this.c.dom.splitDropdownButton.className:"").attr("tabindex",this.s.dt.settings()[0].iTabIndex).attr("aria-controls",this.s.dt.table().node().id).on("click.dtb",function(w){w.preventDefault();!g.hasClass(k.disabled)&&a.action&&t(w,p,g,a);A&&g.trigger("blur")}).on("keypress.dtb",

...
e.collectionTitle+"</div>");e.closeButton&&c.prepend('<div class="dtb-popover-close">x</div>').addClass("dtb-collection-closeable");I(c.insertAfter(h),e.fade);var m=f(b.table().container());d=c.css("position");"dt-container"===e.align&&(h=h.parent(),c.css("width",m.width()));if("absolute"===d){var p=h.position();d=f(b.node()).position();c.css({top:f(f(b[0].node).parent()[0]).hasClass("dt-buttons")?d.top+h.outerHeight():p.top+h.outerHeight(),left:p.left});p=c.outerHeight();var r=m.offset().top+m.height();
r=d.top+h.outerHeight()+p-r;var n=d.top-p,g=m.offset().top;d=d.top-p-5;(r>g-n||e.dropup)&&-d<g&&c.css("top",d);d=m.offset().left;m=m.width();m=d+m;p=c.offset().left;r=c.outerWidth();0===r&&0<c.children().length&&(r=f(c.children()[0]).outerWidth());r=p+r;var t=h.offset().left;g=h.outerWidth();n=t+g;if(c.hasClass(e.rightAlignClassName)||c.hasClass(e.leftAlignClassName)||c.hasClass(e.splitAlignClass)||"dt-container"===e.align){var z=n;h.hasClass("dt-btn-split-wrapper")&&0<h.children("button.dt-btn-split-drop").length&&
(t=h.children("button.dt-btn-split-drop").offset().left,g=h.children("button.dt-btn-split-drop").outerWidth(),z=t+g);g=0;if(c.hasClass(e.rightAlignClassName))g=n-r,d>p+g&&(d-=p+g,m-=r+g,g=d>m?g+m:g+d);else if(c.hasClass(e.splitRightAlignClassName))g=z-r,d>p+g&&(d-=p+g,m-=r+g,g=d>m?g+m:g+d);else if(c.hasClass(e.splitLeftAlignClassName)){if(g=t-p,m<r+g||d>p+g)d-=p+g,m-=r+g,g=d>m?g+m:g+d}else g=d-p,m<r+g&&(d-=p+g,m-=r+g,g=d>m?g+m:g+d)}else d=h.offset().top,g=0,g="button-right"===e.align?n-r:t-p;c.css("left",
c.position().left+g)}else d=c.height()/2,d>f(C).height()/2&&(d=f(C).height()/2),c.css("marginTop",-1*d);e.background&&v.background(!0,e.backgroundClassName,e.fade,e.backgroundHost||h);f("div.dt-button-background").on("click.dtb-collection",function(){});e.autoClose&&setTimeout(function(){b.on("buttons-action.b-internal",function(B,G,D,w){w[0]!==h[0]&&l()})},0);f(c).trigger("buttons-popover.dt");b.on("destroy",l);setTimeout(function(){k=!1;f("body").on("click.dtb-collection",function(B){if(!k){var G=
f.fn.addBack?"addBack":"andSelf",D=f(B.target).parent()[0];(!f(B.target).parents()[G]().filter(a).length&&!f(D).hasClass("dt-buttons")||f(B.target).hasClass("dt-button-background"))&&l()}}).on("keyup.dtb-collection",function(B){27===B.keyCode&&l()})},0)}}});v.background=function(a,b,c,d){c===q&&(c=400);d||(d=x.body);a?I(f("<div/>").addClass(b).css("display","none").insertAfter(d),c):J(f("div."+b),c,function(){f(this).removeClass(b).remove()})};v.instanceSelector=function(a,b){if(a===q||null===a)return f.map(b,
function(e){return e.inst});var c=[],d=f.map(b,function(e){return e.name}),k=function(e){if(Array.isArray(e))for(var h=0,l=e.length;h<l;h++)k(e[h]);else"string"===typeof e?-1!==e.indexOf(",")?k(e.split(",")):(e=f.inArray(e.trim(),d),-1!==e&&c.push(b[e].inst)):"number"===typeof e?c.push(b[e].inst):"object"===typeof e&&c.push(e)};k(a);return c};v.buttonSelector=function(a,b){for(var c=[],d=function(l,m,p){for(var r,n,g=0,t=m.length;g<t;g++)if(r=m[g])n=p!==q?p+g:g+"",l.push({node:r.node,name:r.conf.name,
idx:n}),r.buttons&&d(l,r.buttons,n+"-")},k=function(l,m){var p,r=[];d(r,m.s.buttons);var n=f.map(r,function(g){return g.node});if(Array.isArray(l)||l instanceof f)for(n=0,p=l.length;n<p;n++)k(l[n],m);else if(null===l||l===q||"*"===l)for(n=0,p=r.length;n<p;n++)c.push({inst:m,node:r[n].node});else if("number"===typeof l)m.s.buttons[l]&&c.push({inst:m,node:m.s.buttons[l].node});else if("string"===typeof l)if(-1!==l.indexOf(","))for(r=l.split(","),n=0,p=r.length;n<p;n++)k(r[n].trim(),m);else if(l.match(/^\d+(\-\d+)*$/))n=
f.map(r,function(g){return g.idx}),c.push({inst:m,node:r[f.inArray(l,n)].node});else if(-1!==l.indexOf(":name"))for(l=l.replace(":name",""),n=0,p=r.length;n<p;n++)r[n].name===l&&c.push({inst:m,node:r[n].node});else f(n).filter(l).each(function(){c.push({inst:m,node:this})});else"object"===typeof l&&l.nodeName&&(r=f.inArray(l,n),-1!==r&&c.push({inst:m,node:n[r]}))},e=0,h=a.length;e<h;e++)k(b,a[e]);return c};v.stripData=function(a,b){if("string"!==typeof a)return a;a=a.replace(/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi,
"");a=a.replace(/<!\-\-.*?\-\->/g,"");if(!b||b.stripHtml)a=a.replace(/<[^>]*>/g,"");if(!b||b.trim)a=a.replace(/^\s+|\s+$/g,"");if(!b||b.stripNewlines)a=a.replace(/\n/g," ");if(!b||b.decodeEntities)M.innerHTML=a,a=M.value;return a};v.defaults={buttons:["copy","excel","csv","pdf","print"],name:"main",tabIndex:0,dom:{container:{tag:"div",className:"dt-buttons"},collection:{tag:"div",className:""},button:{tag:"button",className:"dt-button",active:"active",disabled:"disabled",spacerClass:""},buttonLiner:{tag:"span",

...
p=this.s.dt,r=function(w){return"function"===typeof w?w(p,g,a):w};if(a.spacer){var n=f("<span></span>").addClass("dt-button-spacer "+a.style).html(r(a.text));return{conf:a,node:n,inserter:n,buttons:[],inCollection:b,isSplit:c,inSplit:d,collection:null}}!c&&d&&l?k=m:!c&&b&&h.button&&(k=h.button);!c&&d&&l.buttonLiner?e=l.buttonLiner:!c&&b&&h.buttonLiner&&(e=h.buttonLiner);if(a.available&&!a.available(p,a)&&!a.hasOwnProperty("html"))return!1;if(a.hasOwnProperty("html"))var g=f(a.html);else{var t=function(w,
B,E,F){F.action.call(B.button(E),w,B,E,F);f(B.table().node()).triggerHandler("buttons-action.dt",[B.button(E),B,E,F])};h=a.tag||k.tag;var A=a.clickBlurs===q?!0:a.clickBlurs;g=f("<"+h+"/>").addClass(k.className).addClass(d?this.c.dom.splitDropdownButton.className:"").attr("tabindex",this.s.dt.settings()[0].iTabIndex).attr("aria-controls",this.s.dt.table().node().id).on("click.dtb",function(w){w.preventDefault();!g.hasClass(k.disabled)&&a.action&&t(w,p,g,a);A&&g.trigger("blur")}).on("keypress.dtb",
...
this._addKey(z);var G=function(w,B,E,F){y.split.action.call(B.button(f("div.dt-btn-split-wrapper")[0]),w,B,E,F);f(B.table().node()).triggerHandler("buttons-action.dt",[B.button(E),B,E,F]);E.attr("aria-expanded",!0)},D=f('<button class="'+this.c.dom.splitDropdown.className+' dt-button"><span class="dt-btn-split-drop-arrow">'+this.c.dom.splitDropdown.text+"</span></button>").on("click.dtb",function(w){w.preventDefault();w.stopPropagation();!D.hasClass(k.disabled)&&z.action&&G(w,p,D,z);A&&D.trigger("blur")}).on("keypress.dtb",

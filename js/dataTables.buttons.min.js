...
e.popoverTitle+"</div>"):e.collectionTitle&&c.prepend('<div class="dt-button-collection-title">'+e.collectionTitle+"</div>");e.closeButton&&c.prepend('<div class="dtb-popover-close">x</div>').addClass("dtb-collection-closeable");I(c.insertAfter(h),e.fade);var m=f(b.table().container());d=c.css("position");"dt-container"===e.align&&(h=h.parent(),c.css("width",m.width()));if("absolute"===d){var p=h.position();d=f(b.node()).position();c.css({top:f(f(b[0].node).parent()[0]).hasClass("dt-buttons")?d.top+
h.outerHeight():p.top+h.outerHeight(),left:p.left});p=c.outerHeight();var r=m.offset().top+m.height();r=d.top+h.outerHeight()+p-r;var n=d.top-p,g=m.offset().top;d=d.top-p-5;(r>g-n||e.dropup)&&-d<g&&c.css("top",d);d=m.offset().left;m=m.width();m=d+m;p=c.offset().left;r=c.outerWidth();0===r&&0<c.children().length&&(r=f(c.children()[0]).outerWidth());r=p+r;var t=h.offset().left;g=h.outerWidth();n=t+g;if(c.hasClass(e.rightAlignClassName)||c.hasClass(e.leftAlignClassName)||c.hasClass(e.splitAlignClass)||

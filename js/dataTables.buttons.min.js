...
f,f=d>k?f+k:f+d);else if(c.hasClass(h.splitLeftAlignClassName)){if(f=m-n,k<p+f||d>n+f)d-=n+f,k-=p+f,f=d>k?f+k:f+d}else f=d-n,k<p+f&&(d-=n+f,k-=p+f,f=d>k?f+k:f+d)}else d=g.offset().top,f=0,f="button-right"===h.align?r-p:m-n;c.css("left",c.position().left+f)}else d=c.height()/2,d>e(B).height()/2&&(d=e(B).height()/2),c.css("marginTop",-1*d);h.background&&v.background(!0,h.backgroundClassName,h.fade,g);e("div.dt-button-background").on("click.dtb-collection",function(){});h.autoClose&&setTimeout(function(){b.on("buttons-action.b-internal",
function(x,D,G,E){E[0]!==g[0]&&l()})},0);e(c).trigger("buttons-popover.dt");setTimeout(function(){e("body").on("click.dtb-collection",function(x){var D=e.fn.addBack?"addBack":"andSelf",G=e(x.target).parent()[0];(!e(x.target).parents()[D]().filter(a).length&&!e(G).hasClass("dt-buttons")||e(x.target).hasClass("dt-button-background"))&&l()}).on("keyup.dtb-collection",function(x){27===x.keyCode&&l()})},100)}});v.background=function(a,b,c,d){c===q&&(c=400);d||(d=y.body);a?I(e("<div/>").addClass(b).css("display",

...
g.each(d.children,function(a,b){c.appendChild(b)}),d.text&&c.appendChild(a.createTextNode(d.text)));return c}function A(a,b){var d=a.header[b].length,c;a.footer&&a.footer[b].length>d&&(d=a.footer[b].length);for(var e=0,f=a.body.length;e<f&&!(c=a.body[e][b],c=null!==c&&c!==p?c.toString():"",-1!==c.indexOf("\n")?(c=c.split("\n"),c.sort(function(a,b){return b.length-a.length}),c=c[0].length):c=c.length,c>d&&(d=c),40<d);e++);d*=1.3;return 6<d?d:6}function u(a){n===p&&(n=-1===v.serializeToString(g.parseXML(o["xl/worksheets/sheet1.xml"])).indexOf("xmlns:r"));
g.each(a,function(b,d){if(g.isPlainObject(d))u(d);else{if(n){var c=d.childNodes[0],e,f,h=[];for(e=c.attributes.length-1;0<=e;e--){f=c.attributes[e].nodeName;var j=c.attributes[e].nodeValue;-1!==f.indexOf(":")&&(h.push({name:f,value:j}),c.removeAttribute(f))}e=0;for(f=h.length;e<f;e++)j=d.createAttribute(h[e].name.replace(":","_dt_b_namespace_token_")),j.value=h[e].value,c.setAttributeNode(j)}c=v.serializeToString(d);n&&(-1===c.indexOf("<?xml")&&(c='<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'+
...
"");return b===p||!0===b?d+a.extension:d},B=function(a){var b="Sheet1";a.sheetName&&(b=a.sheetName.replace(/[\[\]\*\/\\\?\:]/g,""));return b},r=function(a,b){var d=b.match(/[\s\S]{1,8192}/g)||[];a.clearText();for(var c=0,e=d.length;c<e;c++)a.appendText(d[c])},x=function(a,b){for(var d=b.newline?b.newline:navigator.userAgent.match(/Windows/)?"\r\n":"\n",c=a.buttons.exportData(b.exportOptions),e=b.fieldBoundary,f=b.fieldSeparator,g=RegExp(e,"g"),h=b.escapeChar!==p?b.escapeChar:"\\",j=function(a){for(var b=
"",c=0,d=a.length;c<d;c++)0<c&&(b+=f),b+=e?e+(""+a[c]).replace(g,h+e)+e:a[c];return b},l=b.header?j(c.header)+d:"",k=b.footer&&c.footer?d+j(c.footer):"",m=[],t=0,C=c.body.length;t<C;t++)m.push(j(c.body[t]));return{str:l+m.join(d)+k,rows:m.length}},s={available:function(){return h.hasFlash()},init:function(a,b,d){h.moviePath=i.Buttons.swfPath;var c=new h.Client;c.setHandCursor(!0);c.addEventListener("mouseDown",function(){d._fromFlash=!0;a.button(b[0]).trigger();d._fromFlash=!1});w(c,b);d._flash=c},
destroy:function(a,b,d){d._flash.destroy()},fieldSeparator:",",fieldBoundary:'"',exportOptions:{},title:"*",filename:"*",extension:".csv",header:!0,footer:!1};try{if("undefined"===typeof j.XMLSerializer){var y=function(){};y.prototype.serializeToString=function(a){return a.xml}}var v=new y,n}catch(D){}var o={"_rels/.rels":'<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument" Target="xl/workbook.xml"/></Relationships>',
...
i.Api.register("buttons.resize()",function(){g.each(h.clients,function(a,b){b.domElement!==p&&b.domElement.parentNode&&b.positionElement()})});i.ext.buttons.copyFlash=g.extend({},s,{className:"buttons-copy buttons-flash",text:function(a){return a.i18n("buttons.copy","Copy")},action:function(a,b,d,c){c._fromFlash&&(a=c._flash,d=x(b,c),c=c.customize?c.customize(d.str,c):d.str,a.setAction("copy"),r(a,c),b.buttons.info(b.i18n("buttons.copyTitle","Copy to clipboard"),b.i18n("buttons.copySuccess",{_:"Copied %d rows to clipboard",
1:"Copied 1 row to clipboard"},d.rows),3E3))},fieldSeparator:"\t",fieldBoundary:""});i.ext.buttons.csvFlash=g.extend({},s,{className:"buttons-csv buttons-flash",text:function(a){return a.i18n("buttons.csv","CSV")},action:function(a,b,d,c){a=c._flash;b=x(b,c);b=c.customize?c.customize(b.str,c):b.str;a.setAction("csv");a.setFileName(q(c));r(a,b)},escapeChar:'"'});i.ext.buttons.excelFlash=g.extend({},s,{className:"buttons-excel buttons-flash",text:function(a){return a.i18n("buttons.excel","Excel")},
action:function(a,b,d,c){var a=c._flash,e=0,f=g.parseXML(o["xl/worksheets/sheet1.xml"]),h=f.getElementsByTagName("sheetData")[0],d={_rels:{".rels":g.parseXML(o["_rels/.rels"])},xl:{_rels:{"workbook.xml.rels":g.parseXML(o["xl/_rels/workbook.xml.rels"])},"workbook.xml":g.parseXML(o["xl/workbook.xml"]),"styles.xml":g.parseXML(o["xl/styles.xml"]),worksheets:{"sheet1.xml":f}},"[Content_Types].xml":g.parseXML(o["[Content_Types].xml"])},b=b.buttons.exportData(c.exportOptions),j,l,i=function(a){j=e+1;l=m(f,
"row",{attr:{r:j}});for(var b=0,c=a.length;b<c;b++){for(var d=b,i="";0<=d;)i=String.fromCharCode(d%26+65)+i,d=Math.floor(d/26)-1;var d=i+""+j,k=null;if(!(null===a[b]||a[b]===p||""===a[b])){a[b]=g.trim(a[b]);for(var n=0,o=z.length;n<o;n++)if(i=z[n],a[b].match&&a[b].match(i.match)){k=a[b].replace(/[^\d\.\-]/g,"");i.fmt&&(k=i.fmt(k));k=m(f,"c",{attr:{r:d,s:i.style},children:[m(f,"v",{text:k})]});break}k||("number"===typeof a[b]||a[b].match&&a[b].match(/^-?\d+(\.\d+)?$/)&&!a[b].match(/^0\d+/)?k=m(f,"c",
{attr:{t:"n",r:d},children:[m(f,"v",{text:a[b]})]}):(i=!a[b].replace?a[b]:a[b].replace(/[\x00-\x09\x0B\x0C\x0E-\x1F\x7F-\x9F]/g,""),k=m(f,"c",{attr:{t:"inlineStr",r:d},children:{row:m(f,"is",{children:{row:m(f,"t",{text:i})}})}})));l.appendChild(k)}}h.appendChild(l);e++};g("sheets sheet",d.xl["workbook.xml"]).attr("name",B(c));c.customizeData&&c.customizeData(b);c.header&&(i(b.header,e),g("row c",f).attr("s","2"));for(var k=0,n=b.body.length;k<n;k++)i(b.body[k],e);c.footer&&b.footer&&(i(b.footer,
e),g("row:last c",f).attr("s","2"));i=m(f,"cols");g("worksheet",f).prepend(i);k=0;for(n=b.header.length;k<n;k++)i.appendChild(m(f,"col",{attr:{min:k+1,max:k+1,width:A(b,k),customWidth:1}}));c.customize&&c.customize(d);u(d);a.setAction("excel");a.setFileName(q(c));a.setSheetData(d);r(a,"")},extension:".xlsx"});i.ext.buttons.pdfFlash=g.extend({},s,{className:"buttons-pdf buttons-flash",text:function(a){return a.i18n("buttons.pdf","PDF")},action:function(a,b,d,c){var a=c._flash,e=b.buttons.exportData(c.exportOptions),

...
"row",{attr:{r:j}});for(var b=0,c=a.length;b<c;b++){for(var d=b,i="";0<=d;)i=String.fromCharCode(d%26+65)+i,d=Math.floor(d/26)-1;var d=i+""+j,k=null;if(!(null===a[b]||a[b]===p||""===a[b])){a[b]=g.trim(a[b]);for(var n=0,o=y.length;n<o;n++)if(i=y[n],a[b].match&&!a[b].match(/^0\d+/)&&a[b].match(i.match)){k=a[b].replace(/[^\d\.\-]/g,"");i.fmt&&(k=i.fmt(k));k=m(f,"c",{attr:{r:d,s:i.style},children:[m(f,"v",{text:k})]});break}k||("number"===typeof a[b]||a[b].match&&a[b].match(/^-?\d+(\.\d+)?$/)&&!a[b].match(/^0\d+/)?

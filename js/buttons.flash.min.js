...
g.each(d.children,function(a,b){c.appendChild(b)}),d.text&&c.appendChild(a.createTextNode(d.text)));return c}function z(a,b){var d=a.header[b].length,c;a.footer&&a.footer[b].length>d&&(d=a.footer[b].length);for(var e=0,f=a.body.length;e<f&&!(c=a.body[e][b],c=null!==c&&c!==p?c.toString():"",-1!==c.indexOf("\n")?(c=c.split("\n"),c.sort(function(a,b){return b.length-a.length}),c=c[0].length):c=c.length,c>d&&(d=c),40<d);e++);d*=1.3;return 6<d?d:6}function v(a){n===p&&(n=-1===t.serializeToString(g.parseXML(o["xl/worksheets/sheet1.xml"])).indexOf("xmlns:r"));
...
"row",{attr:{r:j}});for(var b=0,c=a.length;b<c;b++){for(var d=b,i="";0<=d;)i=String.fromCharCode(d%26+65)+i,d=Math.floor(d/26)-1;var d=i+""+j,k=null;if(!(null===a[b]||a[b]===p||""===a[b])){a[b]=g.trim(a[b]);for(var n=0,o=y.length;n<o;n++)if(i=y[n],a[b].match&&a[b].match(i.match)){k=a[b].replace(/[^\d\.\-]/g,"");i.fmt&&(k=i.fmt(k));k=m(f,"c",{attr:{r:d,s:i.style},children:[m(f,"v",{text:k})]});break}k||("number"===typeof a[b]||a[b].match&&a[b].match(/^-?\d+(\.\d+)?$/)&&!a[b].match(/^0\d+/)?k=m(f,"c",

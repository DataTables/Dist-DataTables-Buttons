...
c=a.length;b<c;b++){for(var d=b,g="";0<=d;)g=String.fromCharCode(d%26+65)+g,d=Math.floor(d/26)-1;var d=g+""+m,j=null;if(!(null===a[b]||a[b]===n||""===a[b])){a[b]=i.trim(a[b]);for(var l=0,p=H.length;l<p;l++)if(g=H[l],a[b].match&&a[b].match(g.match)){j=a[b].replace(/[^\d\.\-]/g,"");g.fmt&&(j=g.fmt(j));j=o(f,"c",{attr:{r:d,s:g.style},children:[o(f,"v",{text:j})]});break}j||("number"===typeof a[b]||a[b].match&&a[b].match(/^-?\d+(\.\d+)?$/)&&!a[b].match(/^0\d+/)?j=o(f,"c",{attr:{t:"n",r:d},children:[o(f,
"v",{text:a[b]})]}):(g=!a[b].replace?a[b]:a[b].replace(/[\x00-\x09\x0B\x0C\x0E-\x1F\x7F-\x9F]/g,""),j=o(f,"c",{attr:{t:"inlineStr",r:d},children:{row:o(f,"is",{children:{row:o(f,"t",{text:g})}})}})));k.appendChild(j)}}h.appendChild(k);e++};i("sheets sheet",a.xl["workbook.xml"]).attr("name",M(b));b.customizeData&&b.customizeData(c);b.header&&(d(c.header,e),i("row c",f).attr("s","2"));for(var j=0,l=c.body.length;j<l;j++)d(c.body[j],e);b.footer&&c.footer&&(d(c.footer,e),i("row:last c",f).attr("s","2"));
d=o(f,"cols");i("worksheet",f).prepend(d);j=0;for(l=c.header.length;j<l;j++)d.appendChild(o(f,"col",{attr:{min:j+1,max:j+1,width:I(c,j),customWidth:1}}));b.customize&&b.customize(a);c=new (q||g.JSZip);d={type:"blob",mimeType:"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"};y(c,a);c.generateAsync?c.generateAsync(d).then(function(a){p(a,u(b))}):p(c.generate(d),u(b))},filename:"*",extension:".xlsx",exportOptions:{},header:!0,footer:!1};l.ext.buttons.pdfHtml5={className:"buttons-pdf buttons-html5",
